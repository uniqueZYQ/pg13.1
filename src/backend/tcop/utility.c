				   ...
				   QueryCompletion *qc)
{
	...
	PG_TRY();
	{
		...
		switch (nodeTag(parsetree))
		{
			...
			case T_CreateDeputyClassStmt:
				{
					ObjectAddress	relOid;
					CreateDeputyClassStmt *stmt = (CreateDeputyClassStmt *)parsetree;
					relOid = DefineDeputyClass(stmt, RELKIND_DEPUTYCLASS,InvalidOid,NULL,queryString);
				}
				break;
			...
			case T_CreateForeignTableStmt:
				{
					...
					foreach(l, stmts)
					{
						...
						else if (IsA(stmt, CreateClassStmt))
						{
							CreateClassStmt *cstmt = (CreateClassStmt *) stmt;
							Datum		toast_options;
							static char *validnsps[] = HEAP_RELOPT_NAMESPACES;
...
CreateCommandTag(Node *parsetree)
{
	...
	switch (nodeTag(parsetree))
	{
		...
		case T_CreateClassStmt:
			tag = CMDTAG_CREATE_CLASS;
			break;
			
		case T_CreateDeputyClassStmt:
			tag = CMDTAG_CREATE_SELECTDEPUTYCLASS;
			break;
