...
%type <node>	CreateClassStmt CreateDeputyClassStmt
%type <list>	opt_attr_list  
%type <chr>	deputy_kind
...
%token <keyword> ABORT_P ABSOLUTE_P ACCESS ACTION ADD_P ADMIN AFTER
	...
	GENERATED GLOBAL GRANT GRANTED GREATEST GROUP_P GROUPING GROUPS GROUPDEPUTYCLASS
	...
	UESCAPE UNBOUNDED UNCOMMITTED UNENCRYPTED UNION UNIONDEPUTYCLASS UNIQUE UNKNOWN
...
/*add*/
CreateClassStmt:	CREATE OptTemp CLASS qualified_name '(' OptTableElementList ')'
...
CreateDeputyClassStmt:		CREATE  deputy_kind qualified_name opt_attr_list
					AS select_with_parens   OptTableSpace
				{
...
opt_attr_list:			 '(' OptTableElementList ')'		{$$=$2;}
				|      /*EMPTY*/		        {$$=NIL;}	
		                ;
deputy_kind:		SELECTDEPUTYCLASS			{$$='s';}		
		           | UNIONDEPUTYCLASS			{$$='u';}		
		           | JOINDEPUTYCLASS			       {$$='j';}		
		           | GROUPDEPUTYCLASS			{$$='g';}		
		           ;	
