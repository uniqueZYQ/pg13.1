...
static void
_outDeputyVar(StringInfo str, const Deputy_Var *node)
{
	WRITE_NODE_TYPE("DEPUTYVAR");
...
static void
_outSwitchExprEntry(StringInfo str, const SwitchExprEntry *node)
{
	WRITE_BOOL_FIELD(isRead);
	WRITE_INT_FIELD(exprnum);
	WRITE_INT_FIELD(attrnum);
	WRITE_NODE_FIELD(paths);
	WRITE_NODE_FIELD(SwitchExpr);
}

static void
_outClassAttr(StringInfo str, const ClassAttr *node)
{
	WRITE_OID_FIELD(classid);
	WRITE_INT_FIELD(aggNum);
	WRITE_INT_FIELD(attrnum);
	WRITE_NODE_FIELD(cachedOid);
	WRITE_NODE_FIELD(econtext);
	WRITE_NODE_FIELD(qual);
}

static void
_outCreateClassStmtInfo(StringInfo str, const CreateClassStmt *node)
{
	WRITE_NODE_FIELD(relation);
	WRITE_NODE_FIELD(tableElts);
	WRITE_NODE_FIELD(inhRelations);
...
static void
_outCreateDeputyClassStmtInfo(StringInfo str, const CreateDeputyClassStmt *node)
{
	WRITE_NODE_FIELD(classname);
	WRITE_BOOL_FIELD(isPrecise);
	WRITE_NODE_FIELD(realattrs);
	WRITE_NODE_FIELD(methods);
	WRITE_NODE_FIELD(deputyRule);
	WRITE_NODE_FIELD(createstmt);
	WRITE_CHAR_FIELD(dkind);
	WRITE_NODE_FIELD(writeExprs);
	WRITE_STRING_FIELD(tablespacename);
	WRITE_STRING_FIELD(deputy_desc);
}
...
static void
_outCreateClassStmt(StringInfo str, const CreateClassStmt *node)
{
	WRITE_NODE_TYPE("CREATECLASSSTMT");

	_outCreateClassStmtInfo(str, (const CreateClassStmt *) node);
}

static void
_outCreateDeputyClassStmt(StringInfo str, const CreateDeputyClassStmt *node)
{
	WRITE_NODE_TYPE("CREATEDEPUTYCLASSSTMT");

	_outCreateDeputyClassStmtInfo(str, (const CreateDeputyClassStmt *) node);
}
	...
	else
	{
		...
		switch (nodeTag(obj))
		{
			...
			case T_CreateClassStmt:
				_outCreateClassStmt(str, obj);
				break;
			case T_CreateDeputyClassStmt:
				_outCreateDeputyClassStmt(str, obj);
				break;
			...
			case T_PartitionRangeDatum:
				...
				break;
			case T_SwitchExprEntry:
				_outSwitchExprEntry(str, obj);
				break;


			case T_ClassAttr:
				_outClassAttr(str, obj);
				break;
