...
_copyAConst(const A_Const *from)
{
	...
	switch (from->val.type)
	{
		...
		case T_Null:
			...
			break;
		default:
			elog(ERROR, "d unrecognized node type: %d",
				 (int) from->val.type);
			break;
	}
...
static void
CopyCreateDeputyClassStmtFields(const CreateDeputyClassStmt *from, CreateDeputyClassStmt *newnode)
{
	COPY_NODE_FIELD(classname);
	COPY_SCALAR_FIELD(isPrecise);
	COPY_NODE_FIELD(realattrs);
	COPY_NODE_FIELD(methods);
	COPY_NODE_FIELD(deputyRule);
	COPY_NODE_FIELD(createstmt);
	COPY_SCALAR_FIELD(dkind);
	COPY_NODE_FIELD(writeExprs);
	COPY_SCALAR_FIELD(tablespacename);
	COPY_SCALAR_FIELD(deputy_desc);
}
...
static CreateDeputyClassStmt *
_copyCreateDeputyClassStmt(const CreateDeputyClassStmt *from)
{
	CreateDeputyClassStmt *newnode = makeNode(CreateDeputyClassStmt);

	CopyCreateDeputyClassStmtFields(from, newnode);
...
copyObjectImpl(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_CreateDeputyClassStmt:
			retval = _copyCreateDeputyClassStmt(from);
			break;
