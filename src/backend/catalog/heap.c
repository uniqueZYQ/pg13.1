						 ...
						 ObjectAddress *typaddress)
{
	...
	if (!OidIsValid(relid))
	{
		...
		if (IsBinaryUpgrade &&
			(relkind == RELKIND_RELATION || relkind == RELKIND_SEQUENCE || relkind == RELKIND_CLASS || relkind == RELKIND_DEPUTYCLASS ||
	...
	if (use_user_acl)
	{
		switch (relkind)
		{
			...
			case RELKIND_DEPUTYCLASS:
				...
				relacl = get_user_default_acl(OBJECT_CLASS, ownerid,
											  relnamespace);
				break;
			case RELKIND_SEQUENCE:
