...
ExecuteGrantStmt(GrantStmt *stmt)
{
	...
	else
	{
		...
		foreach(cell, stmt->privileges)
		{
			...
			if (privnode->cols)
			{
				if (stmt->objtype != OBJECT_TABLE && stmt->objtype != OBJECT_CLASS
				)
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_GRANT_OPERATION),
	...
	foreach(cell, nspnames)
	{
		...
		switch (objtype)
		{
			...
			case OBJECT_CLASS:
				...
				objs = getRelationsInNamespace(namespaceId, RELKIND_CLASS);
				objects = list_concat(objects, objs);
...
ExecGrant_Relation(InternalGrant *istmt)
{
	...
	foreach(cell, istmt->objects)
	{
		 ...
		 * checked.
		 */
		if (istmt->objtype == OBJECT_TABLE || istmt->objtype == OBJECT_CLASS)
		{
			if (pg_class_tuple->relkind == RELKIND_SEQUENCE)
			{
				/*
		...
		if (isNull)
		{
			switch (pg_class_tuple->relkind)
			{
				case RELKIND_SEQUENCE:
					...
					break;
				case RELKIND_CLASS:
					old_acl = acldefault(OBJECT_CLASS, ownerId);
					break;
				default:
		...
		case ACLCHECK_NO_PRIV:
			{
				...
				switch (objtype)
				{
					...
					case OBJECT_TABLE:
						...
						break;
					case OBJECT_CLASS:
						msg = gettext_noop("permission denied for class %s");
						break;
					case OBJECT_TABLESPACE:
		...
		case ACLCHECK_NOT_OWNER:
			{
				...
				switch (objtype)
				{
					...
					case OBJECT_TABLE:
						...
						break;
					case OBJECT_CLASS:
						msg = gettext_noop("must be owner of class %s");
						break;
					case OBJECT_TYPE:
	...
	if (isNull)
	{
		...
		switch (classForm->relkind)
		{
			case RELKIND_SEQUENCE:
				...
				break;
			case RELKIND_CLASS:
				acl = acldefault(OBJECT_CLASS, ownerId);
				break;
			default:
